  ...
  handleUploadImage(e) {
    ...
    if (file) {
      ...
      if (this.props.role === "teacher") {
        patchAction(`/teachers/${this.props._id}`, formData, (res) => {
          this.props.onAuth(() => {
      ...
      if (this.props.role === "student") {
        patchAction(`/students/${this.props._id}`, formData, (res) => {
          this.props.onAuth(() => {
            this.props.history.push(`/profile`);
            window.location.reload();
          });
  ...
  render() {
    ...
    let uploadText = this.state.file ? (
      <div>
        ...
        <button
          className="cancel-upload-button btn btn-warning mx-2"
          onClick={this.handleCancelUpload}
        >
          Cancel
        </button>
        <button
          className="upload-button btn btn-primary mx-2"
